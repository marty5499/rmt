...
{
  RMT.apb_conf.apb_fifo_mask = 1;  //enable memory access, instead of FIFO mode.
  ...
  RMT.conf_ch[rmtChannel].conf0.mem_size = 0;
...
void ws2812_handleInterrupt(void *arg)
{
  ...
  else if (RMT.int_st.ch0_tx_end_int_st && ws2812_sem) {
...
void ws2812_init(int gpioNum)
{
  ...
  RMT.tx_lim_ch[RMTCHANNEL].tx_lim = MAX_PULSES;
  RMT.int_ena.ch0_tx_end_int_ena = 1;
  RMT.int_ena.ch0_tx_end_int_ena = 1;
  ...
  esp_intr_alloc(ETS_RMT_CTRL_INUM, 0, ws2812_handleInterrupt, NULL, &rmt_intr_handle);
