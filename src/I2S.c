...
bool initParallelOutputMode(const int *pinMap)
{
	...
	const int deviceBaseIndex[] = {I2S0O_DATA_OUT0_IDX};
	const int deviceClockIndex[] = {I2S0O_BCK_OUT_IDX};
	const int deviceWordSelectIndex[] = {I2S0O_WS_OUT_IDX};
	const periph_module_t deviceModule[] = {PERIPH_I2S0_MODULE};
	...
	const int interruptSource[] = {ETS_I2S0_INTR_SOURCE};
	// ESP_LOGE(TAG_I2s,"in d5");
...
void transpose24x1_noinline(unsigned char *A, uint32_t *B)
{
	...
	t = (x1 ^ (x1 >> 7)) & 0x00AA00AA;
	x1 = x1 ^ t ^ (t << 7);
	t = (x1 ^ (x1 >> 14)) & 0x0000CCCC;
	x1 = x1 ^ t ^ (t << 14);

	t = (x2 ^ (x2 >> 7)) & 0x00AA00AA;
	x2 = x2 ^ t ^ (t << 7);
	t = (x2 ^ (x2 >> 14)) & 0x0000CCCC;
	x2 = x2 ^ t ^ (t << 14);
	...
	t = (y1 ^ (y1 >> 7)) & 0x00AA00AA;
	y1 = y1 ^ t ^ (t << 7);
	t = (y1 ^ (y1 >> 14)) & 0x0000CCCC;
	y1 = y1 ^ t ^ (t << 14);

	t = (y2 ^ (y2 >> 7)) & 0x00AA00AA;
	y2 = y2 ^ t ^ (t << 7);
	t = (y2 ^ (y2 >> 14)) & 0x0000CCCC;
	y2 = y2 ^ t ^ (t << 14);
