...
void WS2812LedBase::Update(uint8_t red, uint8_t green, uint8_t blue, uint8_t white, int LEDIndex, bool show)
{
	if (!_initDone)
		Init();
...
void WS2812LedBase::UpdateAll(uint8_t red, uint8_t green, uint8_t blue, uint8_t white, bool show)
{
	if (!_initDone)
		Init();
...
void WS2812LedBase::Update(RGBColors color, int LEDIndex, bool show)
{
	if (!_initDone)
		Init();
...
void WS2812LedBase::UpdateAll(RGBColors color, bool show)
{
	if (!_initDone)
		Init();
...
void WS2812LedBase::Rotate(int steps, bool directionForward, bool show)
{
...
void WS2812LedBase::Brightness(uint8_t brightness)
{
	if (!_initDone)
		Init();
...
int WS2812LedBase::LedColorType(void)
{
	return _bytesPerColor;
}

int WS2812LedBase::NumberOfLeds(void)
{
	return _numLEDs;
}

void WS2812LedBase::ClearAtExit(bool yesNo)
{
